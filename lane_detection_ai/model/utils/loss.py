...
class ParsingRelationDis(nn.Module):
    ...
    def forward(self, x):
        ...
        embedding = (
            torch.Tensor(np.arange(dim - 1)).float().to(x.device).view(1, -1, 1, 1)
        )
...
def test_cross_entropy():
    ...
    target_one_hot = torch.nn.functional.one_hot(target, num_classes=200).permute(
        0, 3, 1, 2
    )
