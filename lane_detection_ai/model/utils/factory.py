...
from lane_detection_ai.model.utils.loss import (
    MeanLoss,
    ParsingRelationDis,
    ParsingRelationLoss,
    SoftmaxFocalLoss,
...
from lane_detection_ai.model.utils.metrics import AccTopk, Metric_mIoU, MultiLabelAcc
...
def get_loss_dict(cfg):
    ...
    elif cfg.dataset in ["Tusimple", "CULane", "Smartrollerz"]:
        loss_dict = {
            "name": [
                "cls_loss",
                "relation_loss",
    ...
    if cfg.use_aux:
        ...
        loss_dict["op"].append(
            torch.nn.CrossEntropyLoss(
                weight=torch.tensor([0.6, 1.0, 1.0, 1.0, 1.0])
            ).cpu()
        )
    ...
    assert (
        len(loss_dict["name"])
        == len(loss_dict["op"])
        == len(loss_dict["data_src"])
        == len(loss_dict["weight"])
    )
    return loss_dict
...
def get_metric_dict(cfg):
    ...
    assert (
        len(metric_dict["name"])
        == len(metric_dict["op"])
        == len(metric_dict["data_src"])
    )
    return metric_dict
