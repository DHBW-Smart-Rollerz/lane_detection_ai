...
# Add my_interp build directory to Python path to find the compiled .so module
_my_interp_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'my_interp', 'build', 'lib.linux-x86_64-cpython-311')
if not os.path.exists(_my_interp_path):
    ...
    _my_interp_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'my_interp', 'build', f'lib.{plat_specifier}-{python_version}')
if os.path.exists(_my_interp_path):
        ...
        if self.mode == "train":
            if dataset_name == "CULane":
                ...
                cache_path = os.path.join(
                    path, "labels", "smartrollerz_anno_cache.json"
                )
            else:
                raise NotImplementedError
...
def ExternalSourceTrainPipeline(
    ...
    with pipe:
        jpegs, seg_images, labels = fn.external_source(
            source=external_data, num_outputs=3
        )
        ...
        images = fn.noise.gaussian(
            images, mean=0.0, stddev=fn.random.uniform(range=(5.0, 20.0))
        )

        images = fn.noise.salt_and_pepper(
            images,
            salt_val=fn.random.uniform(range=(0.0, 1.0)),
            pepper_val=fn.random.uniform(range=(0.0, 1.0)),
            prob=0.5,
        )

        apply_erase = fn.random.coin_flip(probability=0.4)
        erased_images = fn.erase(
            images,
            anchor=fn.random.uniform(range=(0.0, 0.8), shape=(2,)),
            shape=fn.random.uniform(range=(0.05, 0.2), shape=(2,)),
            axis_names="WH",
            fill_value=0,
            normalized=True,
        )
        images = apply_erase * erased_images + (1 - apply_erase) * images

        apply_white = fn.random.coin_flip(probability=0.4)
        white_images = fn.erase(
            images,
            anchor=fn.random.uniform(range=(0.0, 0.8), shape=(2,)),
            shape=fn.random.uniform(range=(0.05, 0.2), shape=(2,)),
            axis_names="WH",
        ...
        images = fn.resize(
            images, resize_x=train_width, resize_y=int(train_height / top_crop)
        )
        seg_images = fn.resize(
            seg_images,
...
def ExternalSourceValPipeline(
    batch_size, num_threads, device_id, external_data, train_width, train_height
):
    pipe = Pipeline(batch_size, num_threads, device_id)
    with pipe:
        ...
        images = fn.decoders.image(jpegs, device="mixed")
        images = fn.resize(
            images, resize_x=train_width, resize_y=int(train_height / 0.6) + 1
        )
        images = fn.crop_mirror_normalize(
            images,
            dtype=types.FLOAT,
...
class TrainCollect:
    def __init__(
        ...
        if dataset_name == "CurveLanes":
            pipe = ExternalSourceTrainPipeline(
                ...
                nscale_w=self.original_image_width,
                nscale_h=self.original_image_height,
            )
        else:
            pipe = ExternalSourceTrainPipeline(
                batch_size,
        ...
        self.interp_loc_row = (
            torch.tensor(row_anchor, dtype=torch.float32).cuda()
            * self.original_image_height
        )
        self.interp_loc_col = (
            torch.tensor(col_anchor, dtype=torch.float32).cuda()
    ...
    def __next__(self):
        ...
        labels_row = (
            points_row_extend / self.original_image_width * (self.num_cell_row - 1)
        ).long()
        ...
        labels_col = (
            points_col / self.original_image_height * (self.num_cell_col - 1)
        ).long()
...
class TestCollect:
    def __init__(
        self, batch_size, num_threads, data_root, list_path, shard_id, num_shards
    ):
        self.batch_size = batch_size
        eii = LaneExternalIterator(
            data_root,
